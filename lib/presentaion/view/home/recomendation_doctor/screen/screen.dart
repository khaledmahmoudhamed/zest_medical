        ...
        child: Column(
          children: [
            SearchAndIconRow(
              ...
              widget: BlocBuilder<DoctorCubit, DoctorState>(
                builder: (BuildContext context, state) {
                  return IconButton(
                    onPressed: () {
                      BottomSheetClass.showBottomSheet(
                        context,
                        state.allDoctorsList,
                      );
                    },
            ...
            BlocBuilder<DoctorCubit, DoctorState>(
              builder: (BuildContext context, DoctorState state) {
                if (state.recommendedDoctorStatus == DoctorStatus.loading ||
                    state.filterByCityStatus == DoctorStatus.loading ||
                    state.searchDoctorStatus == DoctorStatus.loading) {
                  return Center(child: CircularProgressIndicator());
                } else if (state.recommendedDoctorStatus ==
                        DoctorStatus.loaded &&
                    state.displayedRecommendedDoctorsList.isNotEmpty) {
                  return RecoDoctorCard(
                    ...
                    doctors: state.displayedRecommendedDoctorsList,
                        ...
                        DoctorStatus.failed ||
                    state.filterByCityStatus == DoctorStatus.failed) {
